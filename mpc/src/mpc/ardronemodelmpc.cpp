...
mpc::ArdroneModelMPC::ArdroneModelMPC(ros::NodeHandle node_handle) : nh_(node_handle)
{
	model_ = 0;
	optimizer_ = 0;
	simulator_ = 0;
...
mpc::ArdroneModelMPC::~ArdroneModelMPC()
{
}


bool mpc::ArdroneModelMPC::resetMPC(mpc::model::Model *model, mpc::optimizer::Optimizer *optimizer, mpc::model::Simulator *simulator)
{
...
bool mpc::ArdroneModelMPC::initMPC()
{
	...
	ROS_INFO("Q size: %d",Q_list.size());
	ROS_INFO("Number of states: %d", states_);
	...
	ROS_INFO("ArdroneModelMPC class successfully initialized.");
	return true;
}



void mpc::ArdroneModelMPC::updateMPC(double* x_measured, double* x_reference)
{
	ROS_INFO("1up");
