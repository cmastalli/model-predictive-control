...
double* mpc::example_models::ArDroneSimulator::simulatePlant(double *current_state, double *current_input, double sampling_time)
{
	...
	x_new(0) = x_current(0) + ts * x_current(3);
	x_new(1) = x_current(1) + ts * x_current(4);
	x_new(2) = x_current(2) + ts * x_current(5);
	x_new(3) = x_current(3) + (ts / m_) * (cos(psi) * sin(theta) * cos(phi) + sin(psi) * sin(phi)) * U1;
	x_new(4) = x_current(4) + (ts / m_) * (sin(psi) * sin(theta) * cos(phi) - cos(psi) * sin(phi)) * U1;
	x_new(5) = x_current(5) + (ts / m_) * (-m_ * g_ + cos(theta) * cos(phi) * U1);
	x_new(6) = x_current(6) + ts * (p + q * sin(phi) * tan(theta) + r * cos(phi) * tan(theta));
	x_new(7) = x_current(7) + ts * (q * cos(phi) - r * sin(phi));
	x_new(8) = x_current(8) + ts * (q * sin(phi) + r * cos(phi)) / cos(theta);
	x_new(9) = x_current(9) + ts * ((Iyy_ - Izz_) * q * r / Ixx_ + (d_ / Ixx_) * U2);
	x_new(10) = x_current(10) + ts * ((Izz_ - Ixx_) * p * r / Iyy_ + (d_ / Iyy_) * U3);
	x_new(11) = x_current(11) + ts * ((Ixx_ - Iyy_) * p * q / Izz_ + (1 / Izz_) * U4);
